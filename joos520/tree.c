...
CODE *makeCODEiand(int arg,CODE *next)
{ CODE *c;
  c = NEW(CODE);
  c->kind = iandCK;
  c->visited = 0;
...
CODE *makeCODEixor(int arg,CODE *next)
{ CODE *c;
  c = NEW(CODE);
  c->kind = ixorCK;
  c->visited = 0;
...
CODE *makeCODEishr(int arg,CODE *next)
{ CODE *c;
  c = NEW(CODE);
  c->kind = ishrCK;
  c->visited = 0;
...
CODE *makefromCODEsimplepush(CODE* from, CODE* tail)
{CODE *returnValue = NULL;
	switch (from->kind) {
	  case aloadCK : returnValue = makeCODEaload(from->val.aloadC,tail);
		break;
	  case iloadCK : returnValue = makeCODEiload(from->val.iloadC,tail);
		break;
	  case ldc_intCK : returnValue = makeCODEldc_int(from->val.ldc_intC,tail);
		break;
	  case ldc_stringCK : returnValue = makeCODEldc_string(from->val.ldc_stringC,tail);
		break;
	  case aconst_nullCK : returnValue = makeCODEaconst_null(tail);
		break;
	  default : return NULL;
	}
	return returnValue;
